...
private:
  ...
  std::unique_ptr<VarDeclExprAST> parseDeclaration() {
    ...
    if (lexer.getCurToken() != tok_var)
      return parseError<VarDeclExprAST>("var", "to begin variable declaration");
 ...
 // Type is optional, it can be inferred
  ...
  std::unique_ptr<ExprAST> parseIdentifierExpr() {
     ...
     */
    auto loc=lexer.getLastLocation();
    std::string id=std::string(lexer.getId());
    lexer.consume(tok_identifier);

    if(lexer.getCurToken()!='(')
    {
      return std::make_unique<VariableExprAST>(std::move(loc),id);
    }
    ...
    if(lexer.getCurToken()!=')')
    {
      while(true)
      {
        ...
        if(lexer.getCurToken()==')')
          break;
        if(lexer.getCurToken()!=',')
          return parseError<ExprAST>(")", "to close function call");
        lexer.consume(Token(','));
      }
    }
    lexer.consume(Token(')'));
    ...
    {
      if(args.size()!=1)
        return parseError<ExprAST>("only one argument", "in print function");
      return std::make_unique<PrintExprAST>(std::move(loc),std::move(args[0]));
    }
    return std::make_unique<CallExprAST>(std::move(loc),id,std::move(args));
  ...
  /// Location is retrieved from the lexer to enrich the error messas
