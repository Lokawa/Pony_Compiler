...
private:
  ...
  Token getTok() {
    ...
    if (isalpha(lastChar) || lastChar == '_') {
      ...
      // no continuous '_'
      if (idStr.find("__") != std::string::npos) {
        llvm::errs() << "Identifier <" << idStr << "> contains continuous '_'\n";
        identifierStr ="ERROR_IDENTIFIER";
        return tok_identifier;
      }
      // digit only at the end
      if (idStr.find_first_of("0123456789") != std::string::npos) {
        if (idStr.find_first_not_of("0123456789", idStr.find_first_of("0123456789")) != std::string::npos) {
          llvm::errs() << "Identifier <" << idStr << "> end with number\n";
          identifierStr = "ERROR_IDENTIFIER";
          return tok_identifier;
        }
      }
      ...
      if (idStr.find("_") == 0) {
        llvm::errs() << "Identifier <" << idStr << "> start with '_'\n";
        identifierStr = "ERROR_IDENTIFIER";
        return tok_identifier;
      }
      // check keywords
      if (strcmp(idStr.c_str(), "return") == 0)
      {
        return tok_return;
      }
      if (strcmp(idStr.c_str(), "var") == 0) 
      {
        return tok_var;
      }
      if (strcmp(idStr.c_str(), "def") == 0)
      {
        return tok_def;
      }
      identifierStr = idStr;
    ...
    if (isdigit(lastChar) || lastChar == '.') {
      ...
      if (numStr.find("..") != std::string::npos) {
        llvm::errs() << "Invalid number: " << numStr << "\n";
        identifierStr = "ERROR_NUMBER";
        return tok_identifier;
      }
      // no more than one '.'
      if (numStr.find_first_of(".") != numStr.find_last_of(".")) {
        llvm::errs() << "Invalid number: " << numStr << "\n";
        identifierStr = "ERROR_NUMBER";
        return tok_identifier;
      }
