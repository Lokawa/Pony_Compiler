...
int dumpToken() {
  ...
  while (true) {
    ...
    case Token::tok_identifier:
      tokenName = std::string(lexer.getId());
      break;
    case Token::tok_number:
      tokenName = std::to_string(lexer.getValue());
      tokenName = tokenName.substr(0, tokenName.find_last_not_of('0') + 1);
      if (tokenName.back() == '.') {
        tokenName=tokenName.substr(0, tokenName.size()-1);
      }
      break;
    default:
      tokenName = std::string(1, token);
      break;
    }
