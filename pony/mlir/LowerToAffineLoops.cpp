...
struct GemmOpLowering : public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto ashape = operands[0].getType().cast<MemRefType>().getShape();
    auto bshape = operands[1].getType().cast<MemRefType>().getShape();

    upperBounds[0] = ashape[0];
    upperBounds[1] = bshape[1];
    upperBounds[2] = ashape[1];
    ...
    buildAffineLoopNest(
        ...
        [&](OpBuilder &nestedBuilder, Location loc, ValueRange ivs) {
          ...
          auto loadedA=nestedBuilder.create<AffineLoadOp>(loc,gemmAdaptor.getLhs(),ValueRange{i,k});
          auto loadedB=nestedBuilder.create<AffineLoadOp>(loc,gemmAdaptor.getRhs(),ValueRange{k,j});
          auto loadedC=nestedBuilder.create<AffineLoadOp>(loc,alloc,ValueRange{i,j});
          auto mul=nestedBuilder.create<arith::MulFOp>(loc,loadedA,loadedB);
          auto add=nestedBuilder.create<arith::AddFOp>(loc,loadedC,mul);

          nestedBuilder.create<AffineStoreOp>(loc,add,alloc,ValueRange{i,j});
